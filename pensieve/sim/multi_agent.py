...
S_INFO = 7  # bit_rate, buffer_size, next_chunk_size, bandwidth_measurement(throughput and time), chunk_til_video_end, do_rebuffer
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'w') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            if TEST == 'sensei':  # sensei augmented reward
                # similar linear reward as pensieve, but with future chunks as input and weighted by chunk sensitivity

                # get current video chunk weight
                ...
                reward = (VIDEO_BIT_RATE[bit_rate] / M_IN_K - \
                          ...
                          future_chunk_weight * QOE_PENALTY) * current_weight
            else:  # pensieve reward
                # -- linear reward --
            ...
            #     if rebuf > 0:
            #         # this should be S_INFO number of terms
            #         state[0, -1] = last_state_0
            #         state[1, -1] = last_state_1
            #         state[2, -1] = last_state_2
            #         state[3, -1] = last_state_3 + rebuf
            #         state[4, :A_DIM] = last_state_4
            #         state[5, -1] = last_state_5
            #         continue

            last_state_0 = state[0, -1]
            last_state_1 = state[1, -1]
            last_state_2 = state[1, -1]
            last_state_3 = state[2, -1]
            last_state_4 = state[4, :A_DIM]
            last_state_5 = state[5, -1]

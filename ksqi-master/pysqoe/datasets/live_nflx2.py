...
class LIVENFLX2(StreamingDatabase):
    ...
    def __init__(self, root_dir, version='feature', download=True):
        ...
        self.name = 'LIVE-NFLX-II'
        csv_file = os.path.join(root_dir + "/" + self.name + "/", 'data.csv')
        streaming_log_dir = os.path.join(root_dir + "/" + self.name + "/", 'streaming_logs')
    ...
    def _download(self, root_dir, version='feature'):
        ...
        if not os.path.isdir(root_dir):
            ...
            if all([os.path.isfile(os.path.join(root_dir, 'streaming_logs', s))
                ...
                print('LIVE-NFLX-II already downloaded.')
                return
        
        url = 'http://ivc.uwaterloo.ca/database/LIVENFLX2/'
        filename = 'live_nflx2_feature.zip'
        url += filename

        zip_filename = os.path.join(root_dir, filename)
        if not os.path.isfile(zip_filename):
            print('Downloading LIVE-NFLX-II database...\n')
            download(url, zip_filename)
            print('LIVE-NFLX-II database download complete!\n')

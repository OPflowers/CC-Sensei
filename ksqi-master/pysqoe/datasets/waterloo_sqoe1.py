...
class WaterlooSQoE1(StreamingDatabase):
    ...
    def _download(self, root_dir, version='feature'):
        ...
        if not os.path.isdir(root_dir):
            ...
            if all([os.path.isfile(os.path.join(root_dir, 'streaming_logs', s))
                ...
                print('WaterlooSQoE-I already downloaded.')
                return
        
        url = 'http://ivc.uwaterloo.ca/database/WaterlooSQoE-I/'
        if version == 'full':
            filename = 'waterloo_sqoe_full.zip'
            url += filename
        elif version == 'feature':
            filename = 'waterloo_sqoe_feature.zip'
            url += filename
        else:
            filename = 'waterloo_sqoe_server_video.zip'
            url += filename

        zip_filename = os.path.join(root_dir, filename)
        if not os.path.isfile(zip_filename):
            print('Downloading WaterlooSQoE-I database...\n')
            download(url, zip_filename)
            print('WaterlooSQoE-I database download complete!\n')

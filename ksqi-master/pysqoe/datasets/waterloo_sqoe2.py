...
class WaterlooSQoE2(StreamingDatabase):
    ...
    def _download(self, root_dir, version='feature'):
        ...
        if not os.path.isdir(root_dir):
            ...
            if all([os.path.isfile(os.path.join(root_dir, 'streaming_logs', s))
                ...
                print('WaterlooSQoE-II already downloaded.')
                return
        
        url = 'http://ivc.uwaterloo.ca/database/WaterlooSQoE-II/'
        if version == 'full':
            filename = 'waterloo_sqoe2_full.zip'
            url += filename
        elif version == 'feature':
            filename = 'waterloo_sqoe2_feature.zip'
            url += filename
        else:
            filename = 'waterloo_sqoe2_server_video.zip'
            url += filename

        zip_filename = os.path.join(root_dir, filename)
        if not os.path.isfile(zip_filename):
            print('Downloading WaterlooSQoE-II database...\n')
            download(url, zip_filename)
            print('WaterlooSQoE-II database download complete!\n')

...
def get_augmented_video_qoe(video_no, name):
    ...
    if "phase_2" in name:
        ...
        qoe_index = np.where(np.asarray(video_index_phase_1) == version)
        qoe = np.asarray(video_qoe_phase_1)[qoe_index][0]
...
def get_video_sensitivity_by_chunk(video_no):
    ...
    # abs percent difference from baseline
    # chunks with low error should have weights ~ 1
